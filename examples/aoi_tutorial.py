...
async def main():
    ...
    async with pupilcloud.ApiClient(config) as api_client:
        ...
        try:
            ...
            aoi_colors = {
                "Target": "#32CD32",
                "Other Nietzsche books": "#FF0000",  # Red, originally for "Target"
                "Top Bookshelf": "#D2B48C",  # Tan, the darkest in the pale brown scale
                "Mid Bookshelf": "#D8BFD8",  # Thistle, a light brownish-pink
                "Left books": "#EEDC82",  # Light goldenrod (very light brownish)
                "Right books": "#EEDC82",  # Light goldenrod (same as Left books)
                "Near book L1": "#F5DEB3",  # Wheat (very light brown)
                "Near book R1": "#F5DEB3",  # Wheat (same as Near book L1)
                "Near book L2": "#FFF8DC",  # Cornsilk (pale, almost white brown)
                "Near book R2": "#FFF8DC",  # Cornsilk (same as Near book L2)
            }
            ...
            for aoi in aois.result:
                aoi_formatted = {
                    ...
                    "color": aoi_colors[aoi.name]
                    if aoi.name in aoi_colors
                    else "black",
                }
            ...
            for i, slice in enumerate(
                ...
                if (
                    ...
                    if "Scanning" in recording.result.name:
                        continue
                    logging.info(
